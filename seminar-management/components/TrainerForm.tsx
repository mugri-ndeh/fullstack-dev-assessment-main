  ...
  return (
    <form onSubmit={submit} className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1" htmlFor="t-name">Name *</label>
          <input id="t-name" className={inputCls} value={name} onChange={(e) => setName(e.target.value)} required maxLength={200} />
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1" htmlFor="t-email">Email *</label>
          <input id="t-email" type="email" className={inputCls} value={email} onChange={(e) => setEmail(e.target.value)} required />
        </div>
        <div className="md:col-span-2">
          <label className="block text-sm font-medium text-gray-700 mb-1" htmlFor="t-subjects">
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1" htmlFor="t-location">Location *</label>
          <input id="t-location" className={inputCls} value={location} onChange={(e) => setLocation(e.target.value)} required />
        </div>
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1" htmlFor="t-rate">Hourly rate €</label>
            <input id="t-rate" type="number" min="0" step="0.01" className={inputCls} value={hourlyRate} onChange={(e) => setHourlyRate(e.target.value)} />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1" htmlFor="t-rating">Rating (1–5)</label>
